...
import { addResponseHeaderRule, ECHO_GET_URL, openPopup, ruleRow } from './helpers'

function responseHeaderValue(headers: Record<string, string>, name: string): string | undefined {
  const match = Object.entries(headers).find(([key]) => key.toLowerCase() === name.toLowerCase())
  return match?.[1]
}

test.describe('response headers via echo server', () => {
  test('sets a custom response header on matching responses', async ({ context, extensionId }) => {
