...
import { addHeaderRule, headerValues, openPopup, readEchoHeaders, ruleRow } from './helpers'
...
test.describe('request headers via echo server', () => {
  ...
  test('append of custom header does not inject it on echo', async ({ context, extensionId }) => {
    const popup = await context.newPage()
    await openPopup(popup, extensionId)
